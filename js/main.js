...
function toggleFoodList(close) {
	...
	if (b.style.height !== "480px" && !close) {
		...
		debug.foodListTimeout = setTimeout(() => {
			...
			f.style.pointerEvents = "none";
			document.querySelector("#foodListCount").style.display = "none";
			document.querySelector("#foodListList").style.display = "";
		}, 500);
	}
...
function buy(id) {
	const amt = Number(document.querySelector("#purchaseAmt").value) || 1;
	const p = (foods[id].price !== undefined ? foods[id].price : 5) * (debug.groups.Money.map(g => g.id).includes(id) ? 1 : modifiers.foodPrice) * amt;
	if (debug.locations["counter"] < 20) {
		if (player.money >= p && (typeof foods[id].count === "number" ? foods[id].count : Infinity) >= amt) {
			console.log(amt);
			foods[id].bought += amt;
			if (foods[id].count !== undefined) foods[id].count--;
			debug.selectedItem = items.length;
			items.push(new food(id, false, undefined, 0, foods[id].mass * amt, foods[id].volume * amt, undefined, undefined, undefined, undefined, amt));
...
function createFoodList() {
	...
	let foodGroupList = document.createElement("div");
	foodGroupList.id = "foodGroupList";
	...
	for (const i in foods) {
		...
		foods[i].unavailable = !eval(foods[i].condition);
	...
	for (const g of groupList) {
		...
		e.className = "foodListItem";
		e.innerText = g;
		e.addEventListener("mousedown", () => showGroup(g));
		foodGroupList.append(e);
	}
	foodList.append(foodGroupList);

	for (const j of groupList) {
		const innerGroup = document.createElement("div");
		innerGroup.id = `foodGroup_${j}`;

		const e = document.createElement("h2");
		e.className = "foodListItem";
		e.innerText = j;
		e.addEventListener("mousedown", () => showGroup("none"));
		innerGroup.append(e);
		foodList.append(innerGroup);
		...
		for (let i of debug.groups[j]) {
			...
			// TODO make the onmousedown open the purchase amount menu and then buy, storing the food type in the debug object
			// convert the above to document.createElement-based code because it's easier to read

			const e = document.createElement("p");
			e.className = "foodListItem";
			e.title = foods[i].name;
			e.innerText = foods[i].name +
				(foods[i].mass !== undefined ? ` | ${format("mass", foods[i].mass)}` : "") +
				(foods[i].volume !== undefined ? ` | ${format("volume", foods[i].volume)}` : "") +
				(foods[i].price !== 0 ? ` | ${format("money", (foods[i].price !== undefined ? foods[i].price : 5) * (j !== "Money" ? modifiers.foodPrice : 1))}` : "") +
				(foods[i].count === undefined ? "" : ` | ${format("number", foods[i].count)} Remaining`);
			e.addEventListener("mousedown", () => {
				document.querySelector("#foodListCount > h2").innerText = foods[i].name;
				document.querySelector("#purchaseBtn").innerHTML = `Buy (${format("money", (foods[i].price !== undefined ? foods[i].price : 5) * (j !== "Money" ? modifiers.foodPrice : 1))})`;
				const p = document.querySelector("#purchaseBtn");
				...
				// input amount
				const input = document.querySelector("#purchaseAmt");
				input.value = 1;
				input.addEventListener("input", () => {
					input.value = Math.max(1, Math.min(typeof foods[i].count === "number" ? foods[i].count : Infinity, Math.min(input.value, Math.floor(player.money / foods[i].price * (j !== "Money" ? modifiers.foodPrice : 1)))));
					document.querySelector("#purchaseBtn").innerHTML = `Buy (${format("money", (foods[i].price !== undefined ? foods[i].price : 5) * (j !== "Money" ? modifiers.foodPrice : 1) * input.value)})`;
				});

				document.querySelector("#foodListCount").style.display = "";
				document.querySelector("#foodListList").style.display = "none";
			});
			innerGroup.append(e);
		}
	}
	// set foodListList element's content to foodList
	document.getElementById("foodListList").innerHTML = "";
	document.getElementById("foodListList").append(foodList);

	const backBtn1 = document.createElement("img");
	backBtn1.alt = "Back";
	backBtn1.src = "img/back.png";
	backBtn1.className = "backBtn";
	backBtn1.addEventListener("mousedown", () => {
		if (debug.foodListLocation !== "none") showGroup("none");
		else toggleFoodList();
	});
	document.querySelector("#foodListList").append(backBtn1);
	showGroup("none");

	const backBtn2 = document.createElement("img");
	backBtn2.alt = "Back";
	backBtn2.src = "img/back.png";
	backBtn2.className = "backBtn";
	backBtn2.addEventListener("mousedown", () => {
		document.querySelector("#foodListCount").style.display = "none";
		document.querySelector("#foodListList").style.display = "";
	});
	document.querySelector("#foodListCount").append(backBtn2);
}
...
function clickFunction(i) {
	if (items[debug.selectedItem] !== undefined) {
		if (items[debug.selectedItem].volume !== undefined) {
			...
			if (foods[items[i].id].group === "Money") return; // prevents duplicating money via charcoal
