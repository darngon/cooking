...
setInterval(() => {
    ...
    for (const i in items) {
        if (items[i].location !== "recipeMaker") {
            ...
            output += `<div class="food" id="foodIndex${i}" onmousedown="debug.selectedItem = debug.selectedItem !== ${i} ? ${i} : -1;" oncontextmenu="items.splice(${i}, 1); hideTooltip();" onmouseover="showTooltip('${items[i].custom ? items[i].id : foods[items[i].id].name}', '<p>${Math.round(items[i].temp).toLocaleString()}°F</p><p>${Math.round(items[i].cooked).toLocaleString()}% Cooked</p><p>${items[i].group !== "liquid" ? items[i].format() : ""}</p>');" onmouseout="hideTooltip();"><img style="${(foods[items[i].id].imgAvailable ? '' : 'border-width: 3px; border-style: solid; border-color: white; ')}position: absolute; left: ${left}px; top: ${top}px; filter: brightness(${100 - items[i].cooked / 2}%); drop-shadow(0 0 8px ${tempColor}); ${debug.selectedItem === Number(i) ? "background: #00000060" : ""};" src="img/food/${foods[items[i].id].imgAvailable ? items[i].id : "transparent"}.png" height="${height}" width="${width}" alt="${items[i].id}"></div>`;
